...
__strategy_config = {
    ...
    'symbol_spot_key': 'Nifty Bank',
    'strategy_name': 'Nifty Bank Short Straddle - Premium Level Stop Loss',
    'strategy_code': '202',
    'entry_time': '09:20:00',
    'exit_time': '15:00:00',
    'premium_stop_loss': 1.25,
...
def __data_handler_2(data_feed):
    ...
    try:
        ...
        if 'total_premium' not in __index_data or __index_data['total_premium'] == 0:
            total_premium = 0

            for leg_details in __index_data['legs']:
                total_premium = total_premium + leg_details['entry_price']

            __index_data['total_premium'] = total_premium
            __index_data['premium_stop_loss'] = float(total_premium * __strategy_config['premium_stop_loss'])
        ...
        for leg_details in __index_data['legs']:
            current_premium = current_premium + leg_details['ltp']

        if current_premium > __index_data['premium_stop_loss']:
            __update_status(data_feed, 'Premium Stop Loss Hit')
